...
(function() {
  ...
  Notificator = (function() {
    ...
    Notificator.prototype.notify = function(event, receiver, data, options, callback) {
      ...
      this.log('Notifying: event', event, 'receiver', receiver);
    ...
    Notificator.prototype.notifyDestination = function(event, channelName, destination, data, options, callback) {
      var channel, channelWrap, deferred, wrappedDestination;
      deferred = Q.defer();
      if (typeof data === 'function') {
        callback = data;
        data = options = void 0;
      ...
      this.log('Notifying destination: event', event, 'channel', channelName, ', to', wrappedDestination);
      this.sendMessage(event, channel, null, wrappedDestination, data, function(err, info) {
        if (err) {
          return deferred.reject(err);
        } else {
      ...
      return this.getTemplates(channel, info, (function(_this) {
        return function(err, templates) {
          ...
          return async.forEach(templates, function(template, cb) {
            ...
            _this.log('Notificator: sending message', message, ', to', destination.toString());
