...
(function() {
  ...
  Notificator = (function() {
    ...
    Notificator.prototype.getTemplates = function(event, channel, language, callback) {
      return channel.getTemplates(event, language, function(err, messages) {
        if (err) {
          return callback(err);
        }
      ...
      async.nextTick((function(_this) {
        return function() {
          ...
          return async.forEach(_this.channels, function(channel, cb) {
            var _channel, ref;
            if (util.isArray(options != null ? options.channels : void 0) && (ref = channel.name, indexOf.call(options.channels, ref) < 0)) {
              return cb();
            }
            ...
            return _channel.getDestinations(receiver, function(err, destinations) {
              ...
              return async.forEach(destinations, function(destination, cb2) {
                return _this.getTemplates(event, _channel, destination.language || _this.defaultLanguage, function(err, templates) {
                  if (err) {
                    return cb(err);
                  }
